...
//    // Get password value for validation feedback
...
//             name: data.name,
//             email: data.email,
//             // In production, don't log passwords
//          });
...
//                <motion.form variants={containerVariants} initial="hidden" animate="visible" onSubmit={handleSubmit(onSubmit)} className="text-white bg-white/2 backdrop-blur-md rounded-2xl shadow-2xl p-8 space-y-6 border border-white/20">
...
//                            disabled={isLoading}
//                         />
//                         <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-300 hover:text-white focus:outline-none" disabled={isLoading}>
//                            {showPassword ? (
...
//                               confirmPasswordValue && passwordValue !== confirmPasswordValue ? "border-red-400/50" : "border-gray-400/30"
//                            }`}
//                            placeholder="Password"
//                            disabled={isLoading}
//                         />
//                         <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-300 hover:text-white focus:outline-none" disabled={isLoading}>
//                            {showConfirmPassword ? (
//                               <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
//                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
...
//                      <input id="terms" type="checkbox" className="mt-1 bg-white/10 border border-gray-400/30 rounded focus:ring-2 focus:ring-blue-500" />
...
import { submitAction } from "@/services/auth/register.service";
